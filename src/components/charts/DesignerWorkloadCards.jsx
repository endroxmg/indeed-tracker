      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(activeDesigners.length, 3)}, 1fr)`, gap: 20 }}>
        {designerData.map(({ user, workingDays: wd, expected, logged, pct, diff, ticketsWorked, anyOverridden }) => (
          <div key={user.uid} style={{
            border: '1px solid var(--color-border)', borderRadius: 12, padding: 20, textAlign: 'center',
            background: '#FAFAFA',
          }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 16, position: 'relative' }}>
              <InitialsAvatar name={user.name} size={28} bg="var(--color-primary)" color="#fff" />
              <span style={{ fontFamily: '"Poppins", sans-serif', fontWeight: 600, fontSize: 14, color: '#fff' }}>{user.name}</span>
              {anyOverridden && (
            ...
            <div style={{ position: 'relative', width: 120, height: 120, margin: '0 auto 16px' }}>
              <svg viewBox="0 0 120 120" style={{ transform: 'rotate(-90deg)' }}>
                <circle cx="60" cy="60" r={arcRadius} fill="none" stroke="var(--color-border)" strokeWidth="10" />
                <circle cx="60" cy="60" r={arcRadius} fill="none" stroke="var(--color-primary)" strokeWidth="10"
              ...
              <div style={{
                ...
                fontFamily: '"Poppins", sans-serif', fontWeight: 700, fontSize: 22, color: 'var(--color-primary)',
              }}>
                {pct}%
              </div>
            </div>

            <div style={{ fontSize: 12, color: 'var(--color-secondary-text)', lineHeight: 2, textAlign: 'left', padding: '0 8px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>Working Days:</span>
                <span style={{ fontWeight: 600, color: '#fff' }}>{wd}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>Expected Hours:</span>
                <span style={{ fontWeight: 600, color: '#fff' }}>{expected} hrs</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>Logged Hours:</span>
                <span style={{ fontWeight: 600, color: '#fff' }}>{logged} hrs</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>{diff >= 0 ? 'Extra Hours:' : 'Diff:'}</span>
                <span style={{ fontWeight: 600, color: diff >= 0 ? '#10B981' : '#EF4444' }}>
                  {diff >= 0 ? '+' : ''}{diff} hrs
                </span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>Tickets Worked On:</span>
                <span style={{ fontWeight: 600, color: '#fff' }}>{ticketsWorked}</span>
              </div>
            </div>
          </div>
        ))}
