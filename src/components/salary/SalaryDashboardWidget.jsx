...
export default function SalaryDashboardWidget() {
  ...
  if (loading) return (
    <div style={{ background: 'var(--color-surface)', padding: '24px', borderRadius: 16, border: '1px solid var(--color-border)', height: 280, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  return (
    <div style={{ background: 'var(--color-surface)', borderRadius: 16, border: '1px solid var(--color-border)', display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ background: 'var(--color-primary-light)', width: 32, height: 32, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <IndianRupee size={16} color="var(--color-primary)" />
          </div>
          <div>
            <span style={{ fontSize: 15, fontWeight: 700, display: 'block' }}>Salary Preview</span>
            <span style={{ fontSize: 11, color: 'var(--color-secondary-text)' }}>{format(new Date(), 'MMMM yyyy')}</span>
          </div>
        </div>
      ...
      <div style={{ padding: '20px', flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 12, color: 'var(--color-secondary-text)', marginBottom: 4 }}>Estimated Team Total Net</div>
          <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--color-primary)', fontFamily: '"Poppins"' }}>
      ...
      <button 
        ...
        style={{ 
          ...
          borderTop: '1px solid var(--color-border)', color: 'var(--color-primary)', fontSize: 13, 
      ...
      <div style={{ padding: '8px', textAlign: 'center', fontSize: 10, color: 'var(--color-secondary-text)', fontStyle: 'italic' }}>
